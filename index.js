  ...
  async function tinkoffGetLink(amount, chatId, orderNumber) {
    const TINKOFF_TERMINAL_KEY = '25471612';
    const TINKOFF_TERMINAL_PASSWORD = 'bARtOA';
    ...
    try {
      ...
      if (response.data && response.data.Success === true && response.data.PaymentURL) {
  ...
  app.post('/web-data', async (req, res) => {
    ...
    // После добавления заказа генерируем ссылку на оплату
    const paymentURL = await tinkoffGetLink(totalPrice, chatId, queryId);

    if (paymentURL) {
      await bot.sendMessage(chatId, `Ваш заказ успешно оформлен. Оплатить можно по ссылке: ${paymentURL}`);
    } else {
      await bot.sendMessage(chatId, 'Произошла ошибка при создании ссылки на оплату.');
    }
